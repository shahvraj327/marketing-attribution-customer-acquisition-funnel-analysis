...
SELECT 
	[month_name], revenue, cogs, revenue - cogs as [gross profit], 
	CONCAT(Round(100 * (revenue-cogs) / revenue, 2), '%')    as [Gross Margin pct]
FROM monthly
ORDER By [Month_Name]; 
...
*/
	...
	SELECT
		...
		p.product_id, p.product_name,
		 COUNT(Distinct oi.order_id) as [items sold],
		 COUNT( Distinct oir.order_id) as [items_refunded],
  		 Concat(Round(100 * CAST(COUNT(Distinct oir.order_id)AS FLOAT) /  Count(Distinct oi.order_id), 2) , '%') as [return rate pct]

	FROM flagged ff
	JOIN order_item_refunds oir 
	ON ff.order_item_refund_id = oir.order_item_refund_id
